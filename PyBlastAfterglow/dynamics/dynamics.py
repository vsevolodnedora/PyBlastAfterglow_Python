...
class Driver:
    ...
    def __init__(
            self, r_grid, rhs, vals, pars_ode_rhs, init_v_ns, all_v_ns, **kwargs
        ):
        ...
        for i_v_n, v_n in enumerate(self.init_v_ns):
            init_vals[i_v_n] = self.vals[v_n][0]
    ...
    def _additional_quantities(self, idx):
        ...
        self.vals["beta"][idx] = get_beta(self.vals["Gamma"][idx])
        self.vals["rho"][idx] = self.pars_ode_rhs["rho"] # the one used for evolution
        self.vals["gammaAdi"][idx] = self.eq_gammaAdi(self.vals["Gamma"][idx],
                                                          self.vals["beta"][idx])
        self.vals["rho2"][idx] = self.eq_rho2(self.vals["Gamma"][idx],
                                                  self.vals["beta"][idx],
                                                  self.vals["rho"][idx],
                                                  self.vals["gammaAdi"][idx])

        self.vals["R"][idx] = self.odeinstance.t
        self.vals["tt"][idx] = self.get_elapsed_time(self.vals["R"][:idx],
                                                         self.vals["Gamma"][:idx],
                                                         self.vals["beta"][:idx],
                                                         self.vals["theta"][:idx],
                                                         self.pars_ode_rhs["useSpread"])

        self.vals["thickness"][idx] = self.eq_delta(self.vals["M2"][idx],
                                                        self.vals["rho2"][idx],
                                                        self.vals["theta"][idx],
                                                        self.vals["Gamma"][idx],
                                                        self.vals["R"][idx],
        ...
        self.vals["U_e"][idx] = self.get_U_e(idx)
    ...
    def get_elapsed_time(self, Rs, Gammas, betas, thetas, use_spread):
        ...
        tti = np.trapz(integrand, Rs) + self.vals["tt"][0]
        return tti
