...
class TestElectronDistEvol:
    ...
    def test_plot_evolution_of_electron_crit_lfs(self):
        ...
        shell.dyn = Driver_Nava_FS.from_obj(shell,
                                            useSpread=False, aa=-1., ncells=1, ode='dop853',
                                            ode_pars={"rtol": 1e-4, "nsteps": 1000, "first_step": RR[0]*0.99},
                                            eq_delta="default", eq_dthetadr="None", eq_dmdr="default", eq_gammaAdi="Nava", eq_rho2="rel",
                                            thetaMax=np.pi / 2.,
                                            adiabLoss=True, epsilon_e_rad=0., epsilon_e_rad_RS=0., tprompt=1e3, adiabLoss_RS=True)
                                            # aa=-1, useSpread=False, epsilon_e_rad=0, adiabLoss=True, tprompt=1e3,
                                            ...
                                            # ncells=1)
        shell.dyn_rs = Driver_Nava_FSRS.from_obj(shell,
                                                 useSpread=False, aa=-1., ncells=1, ode='dop853',
                                                 ode_pars={"rtol": 1e-8, "nsteps": 1000, "first_step": RR[0]*0.99},
                                                 eq_delta="default", eq_dthetadr="None", eq_dmdr="default",
                                                 eq_gammaAdi="Nava", eq_rho2="rel",
                                                 thetaMax=np.pi / 2.,
                                                 adiabLoss=True, epsilon_e_rad=0., epsilon_e_rad_RS=0., tprompt=1e3,
                                                 adiabLoss_RS=True)
                                                 # aa=-1, useSpread=False, epsilon_e_rad=0, adiabLoss=True,
                                                 # tprompt=1e3,
                                                 # epsilon_e_rad_RS=0., adiabLoss_RS=True,
        ...
        for i in range(1, len(RR)):
            ...
            shell.electrons.compute_char_lfs_fs(RR[i], shell.dyn)
            shell.electrons2.compute_char_lfs_fs(RR[i], shell.dyn)

            ''' RS '''
            shell.dyn_rs.evolove(RR[i], rho, dlnrho1dR)

            shell.electrons_rs.compute_char_lfs_rs(RR[i], shell.dyn_rs)
            shell.electrons_rs2.compute_char_lfs_rs(RR[i], shell.dyn_rs)
        ...
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons.vals["gamma_min"]), color="blue", label="gamma_min")
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons.vals["gamma_c"]), color="green", label="gamma_c")
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons.vals["gamma_max"]), color="red", label="gamma_max")
        #
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons2.vals["gamma_min"]), color="blue", label="gamma_min 2",
                ls='--')
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons2.vals["gamma_c"]), color="green", label="gamma_c 2",
                ls='--')
        ...
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons_rs.vals["gamma_min"]), color="blue", label="gamma_min")
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons_rs.vals["gamma_c"]), color="green", label="gamma_c")
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons_rs.vals["gamma_max"]), color="red", label="gamma_max")

        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons_rs2.vals["gamma_min"]), color="blue", label="gamma_min",
                ls='--')
        ax.plot(shell.dyn.vals["R"], np.log10(shell.electrons_rs2.vals["gamma_c"]), color="green", label="gamma_c",
                ls='--')
